  ...
  return (
    <div className="min-h-screen bg-[#000000] flex">
      <aside className="w-48 border-r border-[#27272a] bg-[#000000] p-4 flex-shrink-0 hidden md:block">
        <Link href="/dashboard" className="text-xs text-[#71717a] hover:text-[#fafafa] block mb-4">
          ← DASHBOARD
        </Link>
        ...
        {links.map(link => (
          <Link
            key={link.href}
            href={link.href}
            className={"block py-2 px-3 text-xs mb-1 border-l-2 " + (pathname === link.href ? "border-[#ffffff] text-[#ffffff]" : "border-transparent text-[#71717a] hover:text-[#fafafa]")}
          >
            {link.label}
          </Link>
        ))}
