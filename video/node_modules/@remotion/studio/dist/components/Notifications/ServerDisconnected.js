"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.ServerDisconnected = void 0;
const jsx_runtime_1 = require("react/jsx-runtime");
const react_1 = require("react");
const client_id_1 = require("../../helpers/client-id");
const colors_1 = require("../../helpers/colors");
const container = {
    position: 'fixed',
    justifyContent: 'flex-end',
    alignItems: 'flex-start',
    display: 'flex',
    width: '100%',
    height: '100%',
    flexDirection: 'column',
    padding: 30,
    pointerEvents: 'none',
    backgroundColor: colors_1.TRANSPARENT,
    fontFamily: 'SF Pro, Arial, Helvetica, sans-serif',
};
const message = {
    backgroundColor: colors_1.SERVER_DISCONNECTED_BACKGROUND,
    color: colors_1.WHITE,
    paddingLeft: 20,
    paddingRight: 20,
    paddingTop: 12,
    paddingBottom: 12,
    borderRadius: 4,
    boxShadow: colors_1.SERVER_DISCONNECTED_SHADOW,
    lineHeight: 1.5,
};
const inlineCode = {
    fontSize: 16,
    fontFamily: 'monospace',
};
let pageIsGoingToReload = false;
window.addEventListener('beforeunload', () => {
    pageIsGoingToReload = true;
});
const ServerDisconnected = () => {
    const { previewServerState: ctx } = (0, react_1.useContext)(client_id_1.StudioServerConnectionCtx);
    const fav = document.getElementById('__remotion_favicon');
    if (ctx.type !== 'disconnected') {
        fav.setAttribute('href', '/favicon.ico');
        return null;
    }
    if (pageIsGoingToReload) {
        return null;
    }
    const base64Favicon = 'data:image/png;base64,';
    fav.setAttribute('href', base64Favicon);
    return (jsx_runtime_1.jsx("div", { style: container, className: "css-reset", children: jsx_runtime_1.jsxs("div", { style: message, children: ["The studio server has disconnected. ",
                jsx_runtime_1.jsx("br", {}), window.remotion_studioServerCommand ? (jsx_runtime_1.jsxs("span", { children: ["Run", ' ', jsx_runtime_1.jsx("code", { style: inlineCode, children: window.remotion_studioServerCommand }), ' ', "to run it again."] })) : (jsx_runtime_1.jsx("span", { children: "Fast refresh will not work." }))] }) }));
};
exports.ServerDisconnected = ServerDisconnected;
